...
Tokens/Keys
  When needing to use sensitive data in your code (e.g. token) keep in mind that security is paramount. We've used tools in this class to flexibly store configuration information across systems that would work nicely for this. 

Normalization - Eye on the DRY
  Look for duplicated information in your database tables. That is often an indication that a new table is needed.

Set up a playground
  A simple index.html with jQuery linked is a quick and reliable way to test out several kinds of code. JSON, Arrow functions, jQuery syntax, etc. What's value of `` `I’d like to order a { size } soda`` ` Throw the sample code in the playground and proceed with confidence.

`max-width:100%`
  Responsive image scaling for cheap

 In JavaScript properties of an object are available for outside access (aka public) by default. Though there are ways to make data/functions private using a different approach.
